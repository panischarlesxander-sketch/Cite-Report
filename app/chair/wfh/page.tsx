...
import { Clock, CheckCircle2, User, FileText, X, ChevronRight, Eye } from 'lucide-react';
...
interface ApprovalData {
  footer_remarks_1?: string;
  signature_url?: string;
  reviewed_date?: string;
}

export default function ChairWFHPage() {
  const { user } = useAuth();
  ...
  const handleApprove = async (formData?: ApprovalData, reportId?: string) => {
    ...
    try {
      ...
      const updateData: Record<string, unknown> = {
        status: 'Reviewed',
        reviewed_date: formData?.reviewed_date || today
      };
      
      if (formData?.footer_remarks_1) {
        updateData.footer_remarks_1 = formData.footer_remarks_1;
      }
      
      if (formData?.signature_url) {
        updateData.chair_signature = formData.signature_url;
      } else {
        ...
        if (userData?.signature_url) {
          updateData.chair_signature = userData.signature_url;
        }
      }
  ...
  const handleReject = async (reason: string, reportId?: string) => {
    ...
    try {
      const now = new Date().toISOString();
      // If rejecting from table (no reason provided), prompt for one
  ...
  const getStatus = (report: WFHReport) => {
    // Use the explicit status column from the DB
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Reviewed': return 'text-green-600 bg-green-50'; // Display as Approved for Chair
  ...
  const summaryCards = [
    ...
    { label: 'Pending Review', value: stats.pending, icon: Clock, gradient: 'from-amber-400 to-orange-500', shadow: 'shadow-amber-400/25' },
    { label: 'Approved', value: stats.reviewed, icon: CheckCircle2, gradient: 'from-green-500 to-green-600', shadow: 'shadow-green-500/25' },
  ...
  if (viewMode && selectedReport) {
    return (
      <ChairLayout title="Review Report">
            ...
            <div className="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center print:hidden">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 m-4">
                <div className="text-center space-y-2">
                  <LottiePlayer path="/check.json" loop={false} className="mx-auto w-28 h-28" name="chair-review-success" />
  ...
  return (
    <ChairLayout title="WFH Reports">
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 tracking-tight">WFH Accomplishment Reports</h2>
            <p className="text-gray-500 text-sm mt-1">Review submissions and forward approved reports for final dean approval.</p>
          </div>
        </div>

        {/* Tab Navigation */}
        <div className="flex flex-wrap items-center gap-2 p-1 bg-white border border-gray-200 rounded-xl w-fit shadow-sm">
          {(['All', 'Pending', 'Reviewed', 'Rejected'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab === 'Reviewed' ? 'Approved' : tab)}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${
                (activeTab === tab || (activeTab === 'Approved' && tab === 'Reviewed'))
                  ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-md shadow-blue-500/20'
                  : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
              }`}
            >
              {tab === 'Reviewed' ? 'Approved' : tab}
              <span className={`ml-2 px-1.5 py-0.5 rounded-md text-[10px] ${
                (activeTab === tab || (activeTab === 'Approved' && tab === 'Reviewed')) ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-600'
              }`}>
                {tab === 'All' 
                  ? reports.length 
        ...
        <div className="card-soft overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">
              {activeTab === 'All' ? 'Faculty WFH Status' : `${activeTab} Reports`}
            </h2>
          </div>
            ...
            {loading ? (
              <div className="p-12 text-center">
                ...
                if (activeTab === 'All') return true;
                  if (activeTab === 'Pending') return getStatus(r) === 'Pending';
                  if (activeTab === 'Approved') return getStatus(r) === 'Reviewed';
                  return getStatus(r) === activeTab;
              }).length === 0 ? (
              <div className="p-16 text-center">
                <LottiePlayer path="/nocontent.json" className="mx-auto w-44 h-44" name="chair-no-content" />
                    ...
                    <tr key={req.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${getStatusColor(getStatus(req))}`}>
                          {getStatus(req) === 'Reviewed' ? 'Approved' : getStatus(req)}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {getStatus(req) === 'Pending' && (
                            <>
                              <button 
