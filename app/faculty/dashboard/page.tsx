...
export default function FacultyDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      if (data) {
        setStats({
          pending: data.filter(r => r.status === 'Pending' || r.status === 'Reviewed').length,
    ...
    const channel = supabase
      .channel('faculty_dashboard_stats')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'wfh_reports', filter: `user_id=eq.${user.id}` },
        () => {
          fetchStats();
        }
      )
  ...
  const statCards = [
    { label: 'Pending Reports', value: stats.pending, icon: Clock, gradient: 'from-amber-400 to-amber-500', shadow: 'shadow-amber-400/25' },
