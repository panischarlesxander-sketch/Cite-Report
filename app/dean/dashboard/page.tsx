...
export default function DeanDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      if (data) {
        setStats({
          pending: data.filter(r => r.status === 'Reviewed').length, // Dean reviews what Chair has 'Reviewed'
  ...
  const statCards = [
    { label: 'Pending Final Approvals', value: stats.pending, icon: Clock, gradient: 'from-amber-400 to-amber-500', shadow: 'shadow-amber-400/25' },
