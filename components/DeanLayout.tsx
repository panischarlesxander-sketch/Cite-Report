...
export default function DeanLayout({ children, title, showMeta = true }: DeanLayoutProps) {
  const { user, logout } = useAuth();
  const router = useRouter();
  const pathname = usePathname();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const roleLabel = user?.position || 'Dean';
  ...
  const navItems = [
    { name: 'Home', icon: Home, href: '/dean/dashboard' },
    { name: 'WFH', icon: Clock, href: '/dean/wfh' },
    { name: 'E-Signature', icon: UserCheck, href: '/dean/dean' },
    { name: 'Announcement', icon: Megaphone, href: '/dean/announcement' },
    { name: 'Profile', icon: User, href: '/dean/profile' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center gap-6">
              <Link href="/dean/dashboard" className="flex items-center gap-2.5">
