...
export default function ChairLayout({ children, title }: ChairLayoutProps) {
  const { user, logout } = useAuth();
  const router = useRouter();
  const pathname = usePathname();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const roleLabel = user?.position || 'Dept Chair';
  ...
  const navItems = [
    { name: 'Home', icon: Home, href: '/chair/dashboard' },
    { name: 'WFH', icon: Clock, href: '/chair/wfh' },
    { name: 'E-Signature', icon: UserCheck, href: '/chair/chair' },
    { name: 'Profile', icon: User, href: '/chair/profile' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center gap-6">
              <Link href="/chair/dashboard" className="flex items-center gap-2.5">
